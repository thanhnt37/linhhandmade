...
<?php
   $comments = App\CommentPost::select('comment_posts.*','accounts.username','posts.title','posts.slug')->leftJoin('accounts', 'comment_posts.account_id', '=', 'accounts.id')->where('comment_posts.status',1)->leftJoin('posts', 'posts.id', '=', 'comment_posts.post_id')->orderby('created_at','desc')->paginate(10); 
?>
