...
<?php
  if($stt==1){
   $comments = App\CommentFrame::select('comment_frames.*','accounts.username','frames.name')->leftJoin('accounts', 'comment_frames.account_id', '=', 'accounts.id')->where('comment_frames.status',1)->leftJoin('frames', 'frames.id', '=', 'comment_frames.frame_id')->orderby('created_at','desc')->paginate(10); 
  }
?>
 ...
 <div class="app-body" id="view">
    <div class="padding">
      <div class="box">
        ...
        <div class="box-body">
        <form action="{!! route('search.cmpro.done') !!}" method="get">
          <input  style="width:calc(100% - 100px); line-height:30px;" placeholder="Tìm kiếm comment" type="text" class="form-control input-sm inline m-r" name="cm"/>
           <button class="number-post" style="float:right;">{{sizeof($comments)}} BL</button>
        </form>
          ...
          <table class="table m-b-none" ui-jp="footable" data-filter="#filter" data-page-size="10">
            ...
            <tbody>
              ...
              <tr>
                  ...
                  <?php
                    $pro =  App\Frame::select('id','slug')->where('id',$value->frame_id)->first();
                  ?>
                  <td><a style="" href="{{route('getProDetail',['id'=>$value->frame_id,'slug'=>$pro->slug])}}" target="_blank">{{$value->name}}</a></td>
                  <td class="action-post" >
                    <div style="min-width:100px">
                     ...
                     <a href="{!! route('frame.edit.frame',['id'=>$value->frame_id]) !!}">
                      SP
                      </a>
                    </div>
                  </td>
