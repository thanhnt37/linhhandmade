...
<div class="app-header white box-shadow">
    <div class="navbar">
        <div style="float:left;" class="title_form">
            <a href="" id="popup-quantri-themdanhmuc" class="p-themmoi" style="margin-left:0px">
    ...
    <div class="padding">
        ...
        <div class="box">
            <div class="box-body">
                ...
                <button class="btn white dropdown-toggle drop-cate" data-toggle="dropdown" aria-expanded="false"></button>
                <div class="dropdown-menu pull-right">
                    <?php $list_categories = App\GroupAttribute::select('group_attributes.*',"filters.min",'filters.max','filters.value','filters.type')->where(['group_attributes.group_id'=>0])->leftjoin('filters','group_attributes.filter_id','=','filters.id')->get();
                        $space = "-------";
                    ?>
                    <a  class="dropdown-item" href="{{route('product.list.product')}}">Tất cả</a>
                    @foreach($list_categories as $v0)
                    <a  class="dropdown-item" href="{{route('folder.product',['id'=>$v0->id])}}">{{$v0->name}}</a>
                      @if($v0->subcategory)
                           @foreach($v0->subcategory as $v1)
                                <a  class="dropdown-item"  href="{{route('folder.product',['id'=>$v1->id])}}">{{$space}} {{$v1->name}}</a>
                                @if($v1->subcategory)
                                  @foreach($v1->subcategory as $v2)
                                    <a  class="dropdown-item"  href="{{route('folder.product',['id'=>$v2->id])}}">{{$space.$space}} {{$v2->name}}</a>
                                        @if($v2->subcategory)
                                        @foreach($v2->subcategory as $v3)
                                          <a  class="dropdown-item" href="{{route('folder.product',['id'=>$v3->id])}}">{{$space.$space.$space}} {{$v3->name}}</a>
                                          @if($v3->subcategory)
                                             @foreach($v3->subcategory as $v4)
                                              <a  class="dropdown-item" href="{{route('folder.product',['id'=>$v4->id])}}">{{$space.$space.$space.$space}} {{$v4->name}}</a>
                                             @endforeach
                                         @endif
                                         @endforeach
                                     @endif
                ...
                <button class="number-post d-list1">{{sizeof($frames)}} SP</button>
                <div style="clear:both"></div>
            </div>
            <div class="d-dstt" @if(sizeof($attribute_count)) style="border-top:1px solid #F9FAFA;" @endif>
                <?php $name_attr = ""; $list_filter = array();?>
                @foreach($attribute_count as $k => $v)
                    @if( !in_array($v->name, $arr_filter) )
                        @if($v->name != $name_attr)
                            @if($v->avaiable == 1)
                                <?php $list_filter = App\Filter::where('name',$v->name)->get(); ?>
                                @foreach($list_filter as $kx => $filter_2)
                                    <?php $list_filter[$kx]->number_public = 0; ?>
                                    <?php $list_filter[$kx]->number_hidden = 0; ?>
                                @endforeach
                            @endif
                            <div class="d-dstt-row">
                                <p>
                                <span class="d-dstt-row-title">{{$v->name}} :</span>
                        @endif
                        @if(isset($attribute_count[$k+1]))
                            @if($v->avaiable == 1)
                                 @if($attribute_count[$k+1]->name != $attribute_count[$k]->name)
                                        @foreach($list_filter as $kx => $filter_2)
                                            @if( $v->value <= $filter_2->max && $v->value > $filter_2->min )
                                                 <?php  $list_filter[$kx]->number_public =  $list_filter[$kx]->number_public + $v->number_item; ?>
                                                 <?php  $list_filter[$kx]->number_hidden =  $list_filter[$kx]->number_hidden + $v->number_item_hidden; ?>
                                            @endif
                                        @endforeach
                                        @foreach($list_filter as $filter_2)
                                            @if( ( $filter_2->number_public + $filter_2->number_hidden) >0 )
                                            <span class="d-dstt-row-name">@if($filter_2->config_name) {{$filter_2->config_name}} @else {{$filter_2->min}} - {{$filter_2->max}} @endif<span class="slsptt"> [{{$filter_2->number_public}}/{{$filter_2->number_hidden}}]</span></span>
                                            @endif
                                        @endforeach
                            ...
                            @else
                                @if($attribute_count[$k+1]->name != $attribute_count[$k]->name)
                                    <span class="d-dstt-row-name">{{$v->value}}<span class="slsptt"> [{{$v->number_item}}/{{$v->number_item_hidden}}]</span></span>
                                    </p>
                                </div>
                                @else
                                    <span class="d-dstt-row-name">{{$v->value}}<span class="slsptt"> [{{$v->number_item}}/{{$v->number_item_hidden}}]</span></span>
            ...
            <div class="table-responsive">
                <table class="table m-b-none" ui-jp="footable" data-filter="#filter" data-page-size="10">
                    ...
                    <tbody id="list_frame">
                         @foreach($frames as $p_k => $product)
                            <?php $id_product = $product->product_id; $frame_str =  $product->frame_str;?>
                            <?php $list_frame = explode(",",$frame_str);
                              ...
                              $frame_show = App\Frame::whereIn('id',$in)->orderby('updated_at','desc')->get();
                            ?>
                            @foreach($frame_show as $key => $value)

                            <tr @if($p_k%2 == 0) style="background-color:rgb(246,246,246)" @endif>
                            ...
                            <ul class="list-inline comment-pagination">
                              <?php $link_limit = 9; ?>
                                @for ($i = 1; $i <= $frames->lastPage(); $i++)
                                  ...
                                  ?>
                                  @if ($from < $i && $i < $to)
                                      <li class="{{ ($frames->currentPage() == $i) ? ' active' : '' }}">
                                          <a  class="{{ ($frames->currentPage() == $i) ? ' active' : '' }}" href="{{$frames->url($i)}}" data-page={{$i}}>{{ $i }}</a>
                                      </li>
                                  @endif
                                @endfor
                            </ul>
