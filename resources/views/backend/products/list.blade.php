...
?>
 <?php if($cate) { 
        ...
        $list_san_pham  = App\Product::wherein('products.id',$in)->leftJoin('admins', 'products.create_by', '=', 'admins.id')->leftJoin('admins as tbl_edit', 'products.last_edit_by', '=', 'tbl_edit.id')->select('products.*','admins.username as create_by_u','tbl_edit.username as last_edit_by_u')->orderby('products.created_at','desc')->paginate(5);
      }
      else{
        $list_san_pham  = App\Product::leftJoin('admins', 'products.create_by', '=', 'admins.id')->leftJoin('admins as tbl_edit', 'products.last_edit_by', '=', 'tbl_edit.id')->select('products.*','admins.username as create_by_u','tbl_edit.username as last_edit_by_u')->orderby('products.created_at','desc')->take(1000)->paginate(5);
      }
?>
...
<div class="navbar">
    <div style="float:left;" class="title_form">
        @if($cate) 
          ...
          @if(session('admin')->can('them_san_pham') || session('admin')->can('luu_san_pham'))
              <a href="{{ route('product.create.product') }} " class="p-themmoi1">+ Thêm sản phẩm</a>
            @endif
        @else
          <p>
          <!-- phong -->
            @if(session('admin')->can('them_san_pham') || session('admin')->can('luu_san_pham'))
              <a href="{{ route('product.create.product') }} " class="p-themmoi"  style="margin-left:0px">+ Thêm sản phẩm</a>
            @endif
          </p>
        @endif
    ...
    <div style="float:right;line-height:60px;">
       <span class="nav-icon">
          <i class="material-icons" id="d-popup-config-het-hang" style="font-size:25px;">&#xe8b8;</i>
        </span>
    </div>
    @endif
</div>
    ...
    <div class="padding">
      ...
      <div class="box">
        ...
        <div class="d-dstt" @if(sizeof($attribute_count)) style="border-top:1px solid #F9FAFA;" @endif>
                <?php $name_attr = ""; $list_filter = array(); $arr_filter = array()?>
                @foreach($attribute_count as $k => $v)
                        ...
                        @if(isset($attribute_count[$k+1]))
                            ...
                            @if($attribute_count[$k+1]->name != $attribute_count[$k]->name)
                                @foreach($list_filter as $kx => $filter_2)
                                    @if( $v->value <= $filter_2->max && $v->value > $filter_2->min )
                                         <?php  $list_filter[$kx]->number_public =  $list_filter[$kx]->number_public + $v->num; ?>
                                         <?php  $list_filter[$kx]->number_hidden =  $list_filter[$kx]->number_hidden + $v->num_hide; ?>
                                    @endif
                                @endforeach
                          ...
                          @else
                            @if($attribute_count[$k+1]['name'] != $attribute_count[$k]['name'])
                                @if($v->show)
                                <span class="d-dstt-row-name">{{$v['value']}}<span class="slsptt"> [{{$v['num']}}/{{$v['num_hide']}}]</span></span>
                                @endif
                                </p>
                            </div>
                            @else
        ...
        <div class="table-responsive">
            ...
            <tbody id="list_frame">
              @foreach($list_san_pham as $key2 => $value2)
              <?php
                $frame_x = App\Frame::where('frames.product_id',$value2->id)->leftJoin('admins', 'frames.create_by', '=', 'admins.id')->leftJoin('admins as tbl_edit', 'frames.last_edit_by', '=', 'tbl_edit.id')->select('frames.*','admins.username as create_by_u','tbl_edit.username as last_edit_by_u')->get();
              ?>
                @foreach($frame_x as $key => $value)
                <tr @if($key2%2 == 0) style="background-color:rgb(246,246,246)" @endif>
