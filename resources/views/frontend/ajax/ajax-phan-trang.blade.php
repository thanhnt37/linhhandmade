	...
	<div>
		...
		<div class="t-comment-p2">
			<div>
				...
				<?php $comment_cap2 = App\CommentFrame::select('comment_frames.*','accounts.name','accounts.img')->leftJoin('accounts','comment_frames.account_id','=','accounts.id')->where('comment_frames.status',1)->where('comment_frames.frame_id',$product->id)->where('comment_frames.parent_id','=',$comment->id)->orderby('created_at','desc')->take(5)->get();
				?>
